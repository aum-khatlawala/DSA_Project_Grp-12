...
int isCyclicUtil(Graph graph, int src, int *visited, int *recStack)
{   
    int connectedVertex;
    if(visited[src] == 0)
    {
        ...
        Node nodePtr = graph->pFirst[src];
        while(nodePtr!=NULL){
            connectedVertex = nodePtr->data;
            if ( !visited[connectedVertex] && isCyclicUtil(graph,connectedVertex, visited, recStack) )
                return 1;
    ...
    recStack[src] = 0;  // remove the vertex from recursion stack
    return 0;
}

int isCyclic(Graph graph,int N){
    int *visited,*recStack;
    visited = (int *) malloc(sizeof(int)*(N+1));
    recStack = (int *) malloc(sizeof(int)*(N+1));
...
int main()
{
    ...
    */
    while (1)
    {
        ...
        if (strcmp(op, "Find_Cycle") == 0)
        {
            char bankName[10];
            scanf("%s", bankName);
            int key = FindUniqueKey(Hashptr_B, bankName);
            if (!(G[key]))
            {
                    // printf("The bank entered doesn't exist\n");
                    continue;
